...
def calcOden48h(filename,forecastdate='20200417_00',lon0=5,lat0=84):
    '''
    ...
    startdate (in YYYYmmdd_hh) is calculated within the basch script (+48h)
    ...
    '''
    subprocess.call(shlex.split('./%s %s %i %i'%(filename,forecastdate,lon0,lat0)))

def calcOden96h(filename,forecastdate='20200417_00',lon0=5,lat0=84):
    '''
    This function is only for calculating the trajectories. P files required in the same folder as the rundirectory!
    Remember to check that the folder directories in the files used are correct!
    ...
    forecastdate= string of the forecast initialization date
    lon0, lat0 = location of Oden extracted from the sounding BUFR file at 00 UTC
        startdate (in YYYYmmdd_hh) is calculated within the basch script (+96h)
    Duration (in hours) for trajectory computations. Observe that "-" means backward trajectories, eg. -96 for -4d bw
    Variabels along the trajectory will be traced for T and q
    '''
    subprocess.call(shlex.split('./%s %s %i %i'%(filename,forecastdate,lon0,lat0)))
    
def calcLatband70N0h(filename,forecastdate='20200417_00'):
    '''
    This function is only for calculating the trajectories. P files required in the same folder as the rundirectory!
    Remember to check that the folder directories in the files used are correct!
    filename=runLat70N_0h.sh
    forecastdate= string of the forecast initialization date (here also used as the startdate)
    Duration (in hours) for trajectory computations is 96h forward.
    Variabels along the trajectory will be traced for T and q
    '''
    subprocess.call(shlex.split('./%s %s'%(filename,forecastdate)))
    
def calcLatband70N48h(filename,forecastdate='20200417_00'):
    '''
    This function is only for calculating the trajectories. P files required in the same folder as the rundirectory!
    Remember to check that the folder directories in the files used are correct!
    filename=runLat70N_48h.sh
    forecastdate= string of the forecast initialization date; startdate is +48h wrt the forecast date
    Duration (in hours) for trajectory computations is 96h forward.
    Variabels along the trajectory will be traced for T and q
    '''
    subprocess.call(shlex.split('./%s %s'%(filename,forecastdate)))
...
# names: ARTofMELT_forecast_YYYYmmdd_00_fromOden_48h_2dbw, ARTofMELT_forecast_YYYYmmdd_00_fromOden_96h_4dbw, ARTofMELT_forecast_YYYYmmdd_00_70Nlatband_0h_4dfw
...
def strtodatetime_newtime(datestr='20230508_00',format_st='%Y%m%d_%H',hours_dif=-1):
    # this function is not needed anymore when the wanted timestep is computed within the bash script!!!
    '''
    Input: 
...
if __name__ == "__main__":
    ...
    calcOden48h('runOden_48h.sh',Date_forecast,lon0,lat0)
    calcOden96h('runOden_96h.sh',Date_forecast,lon0,lat0)
    #print('calculation done for trajectories initialized at Oden (%s,%s)'%(str(lon0),str(lat0)))

    calcLatband70N0h('runLat70N_0h.sh',Date_forecast)
    calcLatband70N48h('runLat70N_48h.sh',Date_forecast)
